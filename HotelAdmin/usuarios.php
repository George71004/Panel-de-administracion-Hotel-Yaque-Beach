...
<body>
                        ...
                        while($row = $resultado->fetch_assoc()) {
                            echo "<tr>
                                    ...
                                    <td class='table-actions'>
                                        <button class='btn btn-primary btn-sm btn-action edit-btn' data-toggle='modal' data-target='#editUserModal' data-cedula='" . $row['cedula'] . "' data-nombre='" . $row['nombre'] . "' data-apellido='" . $row['apellido'] . "' data-telefono='" . $row['telefono'] . "'>Editar</button>
    ...
    <div class="modal fade" id="editUserModal" tabindex="-1" aria-labelledby="editUserModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editUserModalLabel">Editar Persona</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
    ...
    $('#cedula, #telefono, #editCedula, #editTelefono').on('input', function() {
        this.value = this.value.replace(/[^0-9]/g, '');
    });

    // Validación para solo letras
    $('#nombre, #apellido, #editNombre, #editApellido').on('input', function() {
    ...
    $('.delete-btn').click(function() {
        ...
        $.post('editar_usuario.php', formData, function(response) {
            ...
            if (response.trim() == 'success') {
                ...
                var row = $('#personasTableBody').find('tr').filter(function() {
                    return $(this).find('td').first().text() === formData.id;
                });
                row.find('td').eq(1).text(formData.nombre);
                row.find('td').eq(2).text(formData.apellido);
                row.find('td').eq(3).text(formData.telefono);
